...
def test(model, test_input_handle, configs, itr):
    ...
    for epoch in range(configs.max_epoches):
        ...
        for data in test_input_handle:
            ...
            # Compute FVD over the first 256 samples
            if batch_id <= 15:
                fvd_gt.append(data[:,-output_length:,:].permute(1,0,2,3,4).cpu())
                fvd_gen.append(torch.from_numpy(img_gen[:,-output_length:,:].transpose(1,0,2,3,4)).cpu())
            # print(len(fvd_gen))
            ...
            img = np.ones((2 * res_height,
                           ...
                           configs.img_channel))
            # fvd_data = np.ones((configs.total_length - configs.input_length, ,
            #                configs.img_channel))
            ...
            batch_id = batch_id + 1
        print(len(fvd_gen))
        fvd_gen = torch.cat(fvd_gen, dim=1)
        fvd_gt = torch.cat(fvd_gt, dim=1)
        print(fvd_gen.shape,fvd_gt.shape)
    f.close()
    with codecs.open(res_path + '/data.txt', 'w+') as data_write:
        ...
        # compute fvd
        if configs.fvd == 'True':
            fvd_gen = fvd_gen.clamp(0,1)
            fvd_res = fvd_score(fvd_gt, fvd_gen)
            print('fvd_score: ' + str(fvd_res))
            data_write.writelines(str(fvd_res) + '\n')
