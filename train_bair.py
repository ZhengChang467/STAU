...
if opt.model_dir != '':
    saved_model = torch.load('%s/model_last.pth' % opt.model_dir)
...
if opt.model_dir != '':
    ...
    frame_predictor = lstm_models.stau(opt.g_dim+opt.z_dim, opt.g_dim, opt.rnn_size, opt.predictor_rnn_layers, opt.batch_size, opt.tau, opt.theta)
...
for epoch in range(opt.niter):
    ...
    if epoch % 10 == 0:
        torch.save({
            ...
            'posterior': posterior,
            'opt': opt},
            '%s/model_%s.pth' % (opt.log_dir, str(epoch)))
