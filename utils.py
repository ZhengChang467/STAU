...
def make_image(tensor):
    ...
    #                           high=255*tensor.max(),
    #                           channel_axis=0)
    return tensor.astype(np.uint8)[:,:,[2,1,0]]
...
def save_gif(filename, inputs, duration=0.25):
    ...
    for tensor in inputs:
        ...
        img = img.cpu()
        img = img.transpose(0, 1).transpose(1, 2).clamp(0, 1)*255.
...
def _lpips_wrapper(sample, gt, lpips_model):
    ...
    if img_shape[0] == 1:
        ...
        sample_ = sample
        gt_ = gt
    lpips = lpips_model(sample_.reshape(
        nt * bsz, 3, *img_shape[1:]), gt_.reshape(nt * bsz, 3, *img_shape[1:]))
    return lpips.view(nt, bsz)

def eval_seq(gt, pred, lpips_model):
    # print(len(gt), gt[0].shape, len(pred), pred[0].shape)
    T = len(gt)
    bs = gt[0].shape[0]
    ssim = np.zeros((bs, T))
    psnr = np.zeros((bs, T))
    mse = np.zeros((bs, T))
    x_pred = np.stack(pred, axis=0)
    x_target = np.stack(gt, axis=0)
    x_pred = torch.from_numpy(x_pred).cuda()
    x_target = torch.from_numpy(x_target).cuda()
    lpips = _lpips_wrapper(x_pred, x_target, lpips_model)
    lpips = lpips.detach().cpu().numpy().transpose(1,0)
    # print(lpips.mean())
    for i in range(bs):
        for t in range(T):
            for c in range(gt[t][i].shape[0]):
                ssim[i, t] += compare_ssim(gt[t][i][c], pred[t][i][c])
                psnr[i, t] += compare_psnr(gt[t][i][c], pred[t][i][c])
            ssim[i, t] /= gt[t][i].shape[0]
            psnr[i, t] /= gt[t][i].shape[0]
            mse[i, t] = mse_metric(gt[t][i], pred[t][i])
            # print(i, t, ssim[i, t], psnr[i, t], mse[i, t])
